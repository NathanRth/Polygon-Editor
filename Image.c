...
{
	I_bresenham(img, pos.x - size, pos.y + size, pos.x + size, pos.y + size);
	I_bresenham(img, pos.x + size, pos.y + size, pos.x + size, pos.y - size);
	I_bresenham(img, pos.x + size, pos.y - size, pos.x - size, pos.y - size);
	I_bresenham(img, pos.x - size, pos.y - size, pos.x - size, pos.y + size);
}
...
{
	int xA_10, xB_10, yA_10, yB_10, x_Z2, y_Z2;
	z2_to1erOct(xA, yA, xB, yB, &xA_10, &yA_10, &xB_10, &yB_10);
	int dx = xB_10 - xA_10;
	int dy = yB_10 - yA_10;
	int incr1 = 2 * dy;
	int incr2 = 2 * (dy - dx);
	...
	while (x < xB_10)
	{
		premierOct_to_Z2(xA, yA, xB, yB, x, y, &x_Z2, &y_Z2);
